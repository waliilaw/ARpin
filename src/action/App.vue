  ...
  <vn-app v-show="dataLoaded">
    <div class="header">
      <div class="header-content">
        <div class="wallet-section" v-if="isWalletConnected">
          <span class="wallet-balance">{{ walletBalance }} AR</span>
          <vn-icon-button
            class="wallet-disconnect"
            src="/src/assets/icons/misc/logout.svg"
            :title="getText('buttonTooltip_disconnectWallet')"
...
export default {
  ...
  methods: {
    ...
    async connectWallet() {
      try {
        const address = await arweaveService.connectWallet();
    ...
    async updateWalletBalance() {
      if (this.isWalletConnected) {
        try {
          this.walletBalance = await arweaveService.getWalletBalance(this.walletAddress);
        } catch (error) {
    ...
    async archivePage() {
      ...
      try {
        ...
        const transactionId = await arweaveService.archivePage(pageData);
        this.archiveStatus = 'success';
        
        // Store in IndexedDB for quick access
        await storage.set(`archive:${this.docUrl}`, {
          transactionId,
          timestamp: Date.now()
        });
