...
import { ArweaveWebWallet } from 'arconnect';
...
// Initialize ArConnect
const arconnect = new ArweaveWebWallet();
...
browser.runtime.onMessage.addListener(async (message, sender) => {
  try {
    switch (message.id) {
      case 'archivePage':
        return await archivePage(message.data);
      case 'getArchivedPages':
        return await getArchivedPages();
      case 'connectWallet':
...
// Archive a web page to Arweave
async function archivePage(data) {
  try {
    ...
    const transaction = await arweave.createTransaction({
      ...
      tags: [
        ...
        { name: 'App-Name', value: 'ARpin' },
        { name: 'App-Version', value: '1.0.0' },
        { name: 'Original-URL', value: data.metadata.url },
        { name: 'Title', value: data.metadata.title },
        { name: 'Timestamp', value: data.metadata.timestamp }
      ]
    });
...
async function getArchivedPages() {
  try {
    const walletAddress = await getWalletAddress();
    if (!walletAddress) {
      throw new Error('Wallet not connected');
    ...
    // Get local storage archives
    const storage = await browser.storage.local.get();
    const localArchives = Object.entries(storage)
      .filter(([key]) => key.startsWith('archive_'))
      .map(([_, value]) => value);

    // Query Arweave GraphQL for additional archives
    const query = gql`
      query {
        transactions(
          owners: ["${walletAddress}"]
          tags: [
            { name: "App-Name", values: ["ARpin"] }
          ]
        ) {
          edges {
            node {
    ...
    const arweaveArchives = data.data.transactions.edges.map(edge => ({
      txId: edge.node.id,
      url: edge.node.tags.find(tag => tag.name === 'Original-URL')?.value,
      title: edge.node.tags.find(tag => tag.name === 'Title')?.value,
      timestamp: edge.node.tags.find(tag => tag.name === 'Timestamp')?.value
    }));

    // Combine and deduplicate archives
    const allArchives = [...localArchives, ...arweaveArchives];
    const uniqueArchives = Array.from(new Map(allArchives.map(item => [item.txId, item])).values());
    
    // Sort by timestamp
    return uniqueArchives.sort((a, b) => b.timestamp - a.timestamp);
  } catch (error) {
